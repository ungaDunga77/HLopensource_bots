...
**HL account abstraction modes affect balance reads.** HL now has three modes: default (old behavior), unified (spot+perp combined), and portfolio. In unified/portfolio mode, `clearinghouseState` (what `user_state()` reads) returns zeros — real balance moves to `spotClearinghouseState`. **Bots must check `userAbstraction` at startup and fail fast if mode is unified/portfolio.** HL docs recommend "manual/standard" mode for automated trading. Our testnet account is "default" (fine); mainnet may differ. Added a startup guard.
