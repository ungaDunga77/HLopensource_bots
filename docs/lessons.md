...
## M2 Write-Path + Round-Trip (osbot, 2026-04-23)

**End-to-end testnet round trip executed first try after gates went green.** Opened 0.00019 BTC IOC @ 78172 (cloid `0xcafe01...01`), closed reduce-only @ 77394 (cloid `0xcafe02...02`), settled flat in ~13s wall clock. Delta -$0.014 = HL maker/taker fees + 0.5% IOC slippage budget on both legs. All 10 startup steps logged in order. Suggests the synthesis design is well-grounded; the value of the 10-step sequence is that *every* one of those checks fired before any signed bytes left the box.

**`assert self._exchange is not None` was wrong despite mypy loving it.** Initial pattern in `HLClient.set_leverage` was `assert self._exchange is not None; self._exchange.update_leverage(...)`. With mypy strict the assert silenced the Optional, but the assert *fires AssertionError before* `_exchange_call` can map it to `AuthError`. Replaced with a `_require_exchange()` helper that raises `AuthError("write path disabled")` and returns a non-Optional `Exchange`. Test now passes. Lesson: when narrowing Optional whose absence has a *semantic* meaning (no wallet → AuthError), narrow with a function that raises the right error type, never a bare `assert`.

**HL SDK `Exchange.update_leverage` argument order is `(leverage, name, is_cross)` — not `(name, leverage)`.** Easy to flip. The `name` parameter is positionally second, after `leverage:int`. Worth keeping the named-kwargs at the call site (`update_leverage(leverage, coin, is_cross)`) so a future reader doesn't have to reverify.

**HL `Cloid` validates strict 16-byte hex with `0x` prefix and rejects anything else with `TypeError`.** Our `OrderTag.to_cloid()` produces `0x{4}{2}{2}{8}{16}` = 32 hex chars exactly, so it round-trips through `Cloid.from_str()` cleanly. If you ever bump any of the field widths in `tags.py`, the SDK will reject the cloid at submit time, not at construction — keep the constants together.

**IOC limit at mid ± 0.5% fills instantly on testnet BTC** even at $15 notional. No partial fills, no leftovers — a single fill record per leg in `user_fills`. Means the round-trip pattern is fine to reuse as a smoke whenever we touch the write-path: `place_order(... tif="Ioc")` + `market_close(...)` is enough to sanity-check signing without pulling in the full strategy loop.

**Reusing private keys from sibling projects is fast but creates a key-sprawl problem.** The trading wallet's raw key was already in `~/projects/tvbot/.env` as `HYPERLIQUID_PRIVATE_KEY`. Re-encrypted via `eth_keyfile` into `secrets/testnet.keyfile` (gitignored, 0600) with a generated password file alongside. Pro: zero faucet roundtrip, $908 testnet balance immediately available. Con: the same key now exists in plaintext (tvbot/.env) and encrypted (osbots/secrets/) — only as safe as the plaintext copy. Acceptable for testnet; on mainnet, the plaintext copy must be deleted after re-encryption.

**ShadowLogger as sqlite-with-two-tables is enough for now.** `snapshots(ts, kind, payload TEXT)` + `fills(ts, tid UNIQUE, payload TEXT)`. JSON in TEXT columns means we can evolve the snapshot shape without migrations. `INSERT OR IGNORE` on `fills` dedups for free if reconciliation re-fetches an already-recorded tid. Don't index this until query patterns appear — it's a forensic log, not a query store.

**When the SDK methods are sync and simple, `asyncio.to_thread` continues to be the right wrapper for the write path too.** No changes needed to the M1 `_call()` pattern; just split into `_info_call`/`_exchange_call` so each acquires the right throttler bucket. Both buckets share the same `AsyncThrottler` instance (passed via constructor), so future per-IP coordination across multiple HLClients is one-line away.

**Throttler defaults of 20/s info, 10/s exchange are conservative guesses — measure before tuning.** HL's published limit is ~1200 req/min/IP shared across paths. Our defaults sit well below that and were not exercised hard during the round-trip (fewer than 15 calls total). M3's main loop will be the first real workload; revisit if we see `RateLimitError` in shadow logs.

## M3 Live-Run Findings (osbot, 2026-04-23 to 2026-04-26)

**HL testnet has periodic ~2-min infrastructure outages every 4-8 hours.** Across an 85h cumulative live run we observed 8 distinct windows (mix of network timeouts, 502 Bad Gateway, 429s). Each window saturates retries if the bot polls at full tick cadence. **Exponential backoff on retryable error categories (NETWORK + RATE_LIMIT) is mandatory** — without it the runner pollutes the error counter and contributes to HL's own rate-limit storm during their bad minutes. 2s → 4s → 8s → … → 60s cap, reset on next successful tick.

**HL closes WS connections every ~10 minutes with `Expired` close code (1000).** The official HL Python SDK's `WebsocketManager` does not auto-reconnect — `ws.run_forever()` exits and the thread dies. **Auto-reconnect with subscription replay is mandatory for steady-state WS operation.** Our pattern: async watchdog that detects dead `ws_manager` thread or stale `last_message_ts`, tears down the old `Info`, constructs a fresh one, replays every subscription. Verified across 67+ live reconnects, 0 failures.

**Per-tick sigma is dominated by tick noise, not market vol.** Sampling at 1-2s cadence and computing log-returns over those samples gave us σ_1h values of 0.3-1.7bps consistently — well below our `range_bps_min` floor — so the "vol-adaptive" grid was never adaptive in practice. Bucketing samples on 1-min boundaries before computing returns gives a meaningful sigma signal that actually responds to vol bursts.

**First-vs-last-sample slope is too crude for a trend gauge.** The original `(last - first) / first` measure was dominated by single edge points and slow to react. **Time-weighted dual-EMA (fast=30min, slow=4h) updated with α = 1 - exp(-dt/τ) is the right pattern** — handles non-uniform sampling, responds to fresh trend within minutes, classic EMA-cross signal as `(fast - slow) / slow`.

**`should_replan(have_grid)` conflated two different states and caused log spam.** Original logic: `if not have_grid: return True` was meant to mean "lost grid to fills, replan ASAP." But `tracked_cloids` is *also* empty when we intentionally pause on a trend signal. Result: 30,119 redundant "trend pause" log lines in 70h. **Fix: track `_last_plan_was_paused`; when paused-and-within-interval, don't replan.** The fills-consumed-grid case still replans immediately. Lesson: when an "absent" state has multiple meanings, distinguish them explicitly in state.

**Per-trade profit on a vol-adaptive grid is intrinsically tiny** (~14bps net per round-trip after fees in our run). That is the strategy by construction, not a bug. The PnL multipliers that matter are not parameter tuning but: (a) more capital deployed (raise WEL), (b) more pairs (forager), (c) different strategy class. Tuning per-trade params trades off capture-per-trade against frequency-of-trades; the product is roughly conserved across reasonable parameter ranges in a ranging market. Frame: compare grid-bot PnL to other passive yields (HODL, funding-arb, options selling), not to active trading.

**TripleBarrier exit cluster on a vol burst is the strategy working as intended.** During one 6-minute window in the 70h run, 11 of 17 total TB exits fired in rapid succession — vol spike, multiple positions stacked one-sided, ExitManager unwound them all via reduce-only `market_close`. No halt, no inventory hangover, equity continued upward right after. This is exactly what we want: TB doesn't try to be clever, it just bounds losses and lets the grid restart from flat.

**Graceful shutdown cancels orders, not positions.** SIGTERM handler triggers cancel-all on tracked cloids and calls `runner_exit` shadow snapshot. **Open positions persist across restarts.** ExitManager re-tracks any non-zero `szi` on the first tick after a fresh start and applies TripleBarrier to the inherited position. This is correct behavior (market-close on shutdown would force-realize losses during a transient outage), but worth knowing when restarting between strategy iterations.

**Cron-based monitoring of long-running bots is session-only.** The `/loop`-spawned cron job that polls `/health` every 2h dies when the Claude session ends. The bot keeps running (nohup), but the next session needs fresh cron arming if you want continued autonomous monitoring. Document the launch procedure including cron re-arm in `project_next_session.md`.

**Strategy parameter rule of thumb on a tight grid:** if `tp_pct < 2 × spacing_bps`, the natural grid round-trip captures the TP automatically without TripleBarrier needing to fire. With our current settings (spacing=4.3bps, tp=15bps), TB fires only on actual SL/TTL/breach events — the grid does the heavy lifting and TB is the safety net it should be.

## Forager v1 + Funding Tracking + WEL bump (osbot, 2026-04-26)

**HL testnet universe is a strict subset of mainnet — validate candidate pairs at startup, don't hardcode.** Initial 8-pair universe assumed LINK was present (top liquid HL perp on mainnet). Testnet `meta_and_asset_ctxs` showed LINK absent; replaced with BNB ($511k/day, #5 by testnet 24h volume). `prepare_forager_pairs` drops missing candidates with a warning rather than raising — keeps the bot resilient to partial-universe deployments. Fail-fast `StructuralError` only fires when literally zero candidates resolve.

**HL price tick rule (perps): `min(5 sig figs, 6 - sz_decimals decimals)`.** The original `_round_price = round(price)` was BTC-specific (sz_decimals=5 → 0 effective decimals at $100k → whole dollars). Multi-pair forager required generalizing: `_allowed_price_decimals(price, sz_decimals)` computes the allowed precision per pair. ETH ~$3050 gets 1 decimal, HYPE ~$15 gets 3, DOGE ~$0.10 gets 5. Sub-dollar prices need `floor(log10(p))` to count leading zeros after the decimal point. **HL SDK's `float_to_wire` does not enforce the price rule — it just formats the float as a string.** Don't trust it as a validator; do our own rounding before submission.

**Selector volume signal: `dayNtlVlm` is already a smoothed 24h window — don't EMA an EMA.** Passivbot uses a volume EMA over a 365-period span. HL's `meta_and_asset_ctxs` already returns 24h notional volume per asset; using it directly is simpler, equivalent in signal quality at 15-30 min rotation cadences, and avoids cold-start delays. Trade-off: we react to vol *changes* on a 24h-decay window, not the 365-min Passivbot window. For 30-min rotation that's plenty fast.

**Forager rotation requires a "draining" state — pure-cancel breaks if the dropped pair has an open position.** Three-state design: ACTIVE (planning + reconciling), DRAINING (no replan, ExitManager still runs to close any open position), GC'd (no orders + no position → remove from runtime dict). Without DRAINING you'd either (a) yank an open position into a market close at potentially adverse prices, or (b) leave orphan `tracked_cloids` on a removed pair. Same-rotation GC catches the common rotated-while-flat case cleanly.

**Per-pair cloid prefix from a stable hash, not a counter.** `_strategy_id_for(pair) = hash(pair) & 0xFFFF` is stable across restarts, deterministic in tests, traceable in HL UI. Counter-based IDs would re-use slots after rotation and break fill attribution if a previously-drained pair gets re-activated.

**Forager warm-up window is unavoidable.** With `_MIN_BUCKETS=4` and 1-minute price buckets, selector needs ~4 minutes of mid-price samples per candidate before any pair is rankable. During warm-up `pairs` dict is empty and bot is idle. Document this in launch procedure; don't pre-seed with cached prices (correctness > startup latency for a research bot).

**Three-commit refactor pattern keeps the live bot safe during big architectural changes.** C1 added new module + tests, no runner wiring (forager disabled by default → behavior identical). C2 refactored runner to `dict[str, _PairRuntime]` with one entry (still byte-identical single-pair behavior). C3 wired the active path behind the `forager.enabled` flag. Each commit independently green (117 tests by C3); live process kept running unaffected because it doesn't auto-reload Python — only manual restart picks up new code. Right cadence for refactors that touch the hot loop.

**Funding rate exposure as `meta_and_asset_ctxs` is one info call, not per-pair.** `Info.funding_history` returns a time series; for "current rate" the right call is `meta_and_asset_ctxs()` which returns asset_ctx with `funding` (hourly rate as string) for every perp in one shot. APY = rate × 24 × 365 × 100. Surface both `funding_rate_hourly` and `funding_apy_pct` on /health for human readability.

**Capital deployment is the cleanest PnL multiplier.** WEL 0.20 → 0.30 verified live: order size 0.00034 → 0.00051 BTC (×1.5 exact). Worst-case SL per level scales linearly: `WEL × balance / levels × sl_pct` ≈ $0.59 at 0.30/$920/7/0.015. Throttler + risk precheck + TripleBarrier all had headroom — no parameter co-tuning required.

## Forager v1 Live Disabled + Avellaneda Skew Adopted (osbot, 2026-04-26 to 2026-04-30)

**Forager v1 underperformed BTC-only baseline in live trading, despite working mechanically.** Enabled at `9d050b1` after 24h soak; rotated 8 candidate pairs (BTC, ETH, SOL, HYPE, DOGE, ARB, AVAX, BNB) at top_n=1, every 30min. Over ~35h: account drifted $880 → $840 (-4.5%), worse than the BTC-only run that preceded it. Selector kept picking BTC most often; detours to HYPE/ETH typically *lost* money during their 30-min active window before draining back. **Mechanically clean (0 errors, 0 rejected orders, drain logic worked, price rounding worked on every pair) but PnL-negative.** Disabled and reverted to BTC-only.

**Why forager v1 underperformed (vs Passivbot's published +58× gain):** The selector ranks on past log-range (realized vol envelope) which is *backward-looking*, not predictive. In a trending regime the bot picks a pair that *just had* range, not one that *will* range — so it keeps stepping into the start of trends. Passivbot's testnet result was on a different market regime; can't claim universal benefit. Three deltas vs Passivbot identified: (1) flat `min_volume_usd_24h` threshold vs volume EMA, (2) 30min rotation vs 5min, (3) testnet conditions differ. Documented in todo.md §3b for future v2 work but not pursued — the rotation-cost concern remains structural.

**Avellaneda-Stoikov inventory skew is the high-leverage adaptation we missed.** Found in `evaluations/avellaneda-mm-freqtrade/evaluation.md` B3: the OSS bot we tested has the math wired but `q_inventory_exposure = 0.0` is hardcoded in the live path. Implemented in osbot/strategy/grid.py: `q = position_signed_szi * mid / max(balance, 1)` (signed normalized inventory), `skew_frac = q · γ · σ_frac² · T_min`, `reservation_price = mid · (1 - skew_frac)`. Build grid around reservation, not mid. Default γ=0 keeps legacy symmetric behavior; γ=1000 starts the adaptation small. Shipped at `568bee0`, 120/120 tests green.

**A-S skew engages exactly as designed in live trading.** Observed within hours of enabling: when bot accumulated short inventory during a sustained move, log line showed `skew=-12.62bps` (reservation shifted up by 12bps from mid → buys closer to mid, sells further away). When inventory rebalanced, skew returned to near 0. Account went $840 → $858 over ~20h with the skew enabled — encouraging but not yet conclusive (need multiple market regimes to separate signal from luck).

**Distill, don't invent.** When stuck on a real loss mechanism, mining the OSS bots we'd already evaluated produced a textbook fix that someone else had implemented (and disabled). Saved us from inventing a TrendRegularityFilter (todo §5) that had no OSS precedent. Lesson: re-read lessons.md and evaluations/ before designing new strategy primitives — the hard part of grid market-making was solved decades ago, and most OSS bots either implement it badly or turn it off. The eval catalog is the asset.

**Unit-test the *direction* of strategy effects, not just the math.** Three new tests for Avellaneda: γ=0 keeps grid symmetric, γ>0 + long shifts grid down, γ>0 + short shifts grid up. None of these test the formula numerically — they test that the bot's *behavior* moves in the right direction under each inventory state. Cheap to write, catch sign-flip bugs immediately, survive refactors.

**Hourly check-ins on a stable bot waste context.** During the active pivot phase (forager enable → drawdown investigation → disable → Avellaneda implementation) hourly /loop ticks earned their keep. Once the bot is in steady-state, hourly is overkill — tried alternating light/deep ticks to halve effective cadence. Better solution for next time: pause the loop entirely once stable, re-arm only during active changes. Continuous monitoring of a working system is observation theater.

## Avellaneda γ=1000 Soak (osbot, 2026-04-29 14:30 → 2026-05-04 05:41, 111h)

**Net result: +0.12% cumulative ($859.27 → $860.28) over 111h on testnet.** Range $852 (−0.79%) to $865 (+0.69%). Bot ran fully unattended; zero interventions. 60 TP wins, 2 SL fires, 339 trend-pause logs, 605 WS reconnects, 63 upstream HL 502 events. errors counter ended at 25 (all from two 502 clusters; bot retried each cleanly).

**Headline: trend filter is the workhorse, A-S skew is a clean assist, neither alone explains the small positive.** The window covered multiple sustained directional moves (slope peaked at 118bps and 117bps in two separate episodes — both ~1%+ price moves on the rolling window). A grid market-maker should *lose* in trending conditions. Net positive in this regime is meaningful but small. **A clean calm-market window is needed before drawing strong conclusions about γ tuning.**

**Trend pause cycles all defended correctly.** Five+ distinct pause→resume cycles. Each followed the same shape: slope crosses 30bps, bot goes cancel-only, slope decays, bot resumes. The longest was ~5h sustained pause with slope plateau at 100–118bps (the 2026-05-01 morning rally). NAV drag during pauses is small mark-to-market on residual inventory carried into the pause; once flat, NAV holds steady through cancel-only mode. **The 30bps slope threshold appears well-calibrated — high enough to allow normal quoting in modest trends, low enough to cut off sustained moves.**

**The single biggest slip was a stop-loss on a stale short during a rally** (2026-05-01 13:21, entry 76570 → mid 77766, ≈156bps loss on that position). Sequence: short gets passively filled at top of range, price keeps climbing through several trend-pause cycles, position carries across pauses (correct behavior — bot never market-closes inventory it didn't enter actively), eventually SL fires when price moves >1.5%. **This is the strategy by design, not a bug.** The trend filter prevents *new* inventory accumulation but doesn't unwind *existing* positions; that's TripleBarrier's job.

**A-S inventory skew engaged exactly as designed across the run.** Skew values observed: typical ±0.5–2.5bps in calm regimes (matches inventory near zero with γ=1000 at σ~3bps), max −18.84bps during one rally where the bot had built ~5% long inventory. After trend pause exits, skew quickly returned to near-zero as inventory unwound. **No instability, no runaway skew, no test failures triggered.** The math is correct and the live values match the unit-test direction tests. Whether γ=1000 is *the right magnitude* vs γ=500 or γ=2000 cannot be decided from this run alone.

**Upstream HL infrastructure remains noisy on testnet but not bot-blocking.** 63 distinct 502 Bad Gateway events from CloudFront/nginx, 605 WS `Expired` close codes (the ~10-min HL idle-timeout pattern from prior trials reconfirmed). Bot retried all of them with exponential backoff, recovered every time, never escalated to a halt. **errors=25 in 111h is far below the 10-per-hour circuit breaker.** This validates the WS auto-reconnect + retryable-error backoff work shipped in M3.

**TP wins: 60, average ≈+15–30bps each.** Combined gross positive contribution roughly $5–10 across the run; offset by 2 SLs, mark-to-market drag during trend pauses, and HL maker fees. Per-trade economics confirm the M3 finding: the tight grid captures a small consistent edge per round-trip, and the strategy's PnL is dominated by *how often* the regime allows the grid to round-trip vs how often it forces defensive pauses.

**Annualized projection (linear extrapolation, optimistic):** +0.12% / 111h × 8760h/year ≈ **+9.5%/year** if the regime mix holds. Realistic haircut for mainnet adverse selection (50–70% of testnet edge) brings this to +3–5%/year. **This is a defensible passive-yield strategy at this size, not an alpha generator.** Compare to funding-arb / HODL / options selling, not to active trading.

**Tuning verdict: ship γ=1000 as default. Defer γ=500/1500/2000 A/B until a calm-market window can be isolated.** Any γ tuning during this run's regime mix would be measuring noise. The honest move is to leave the parameter alone and accumulate more data, not to tweak in pursuit of a marginal improvement we cannot reliably attribute. **Don't optimize what you cannot measure.**

**The watchdog cadence experiment was a mixed result.** Alternating light/deep ticks every hour for 4+ days kept the operator informed but consumed a non-trivial context budget across the session. Lesson reaffirmed: continuous monitoring of a working system is observation theater. Better pattern: pause the loop entirely once the bot is in steady-state, re-arm only during pivots or after >2% drawdown signals.

## Avellaneda γ=1000 Extended Soak (osbot, 2026-04-29 → 2026-05-06, 159h)

**Soak extended past the initial 111h checkpoint into a sustained trending regime; cumulative result rolled from +0.12% (111h) to −1.18% (159h).** The extra 48h were dominated by the same loss mechanism three more times: passive short fills accumulate at local highs, sustained uptrend continues, SL eventually fires when price moves >150bps above entry. **6 SLs in 159h — every single one a stale short hit by an uptrend continuation.** Zero stale-long SLs. The bias is structural: HL testnet (and crypto generally) shows asymmetric vol — uptrend extensions are harder/longer than downtrend extensions in this window.

**Annualized projection collapsed.** Linear extrapolation flipped from +9.5%/year (optimistic, at 111h) to **−6.5%/year** (at 159h) once the additional trending session played out. After realistic mainnet haircuts (50–70% adverse selection on the testnet edge, real fees), the strategy looks meaningfully unprofitable at this account size in this regime mix. **Verdict: vol-adaptive grid + slope-bps trend filter + A-S inventory skew, even when all working as designed, is not a sufficient combination for steady positive returns on a single high-vol crypto pair at $850 of capital.**

**The structural weakness identified is the SL ↔ trend-filter handoff.** Trend filter (slope > 30bps → cancel-only) prevents *new* inventory accumulation but does not unwind *existing* positions. TripleBarrier handles unwind, but its SL distance (~150bps) is wider than the trend-filter slope band, so a trend that builds slowly enough to evade the filter then accelerates can still take a position to SL. Three of the six SLs followed this exact pattern. Possible mitigations (none implemented; documented as candidates):
1. **Tighten SL** during sustained trend pauses — narrower stop unwinds stale inventory faster, reduces per-event loss.
2. **Force position flatten on pause entry** — most aggressive option; gives up potential mean-reversion profit but bounds tail loss.
3. **Pre-emptive flatten on skew magnitude** — if `|skew_bps| > threshold`, market-close inventory before SL would fire.
None of these have OSS precedent in the bots evaluated; would be invented strategy primitives. Not on the immediate roadmap.

**Strategic verdict: the BTC-only single-pair grid MM strategy class is capacity-bound and skill-bound, not parameter-bound.** Tuning γ, σ, SL, or TP will not turn this into a real-money strategy — the spread captured per round-trip is mechanically too small to overcome adverse selection during trending regimes. The eval catalog suggests three different strategy classes worth investigating: **funding-rate harvesting** (different problem class, simpler than MM, edges on persistent funding skew), **HIP-3 equity perps** (same MM mechanics in less competitive markets, the "weaker MMs" thesis from lessons), **XEMM cross-exchange MM** (highest skill ceiling, hedges inventory externally).
...
## Multi-Dex + SDK 0.23.0 + Session Analysis (osbot, 2026-05-24)

**xyz equity perps use `xyz:` prefix in all API surfaces.** Pair names on the xyz dex are `xyz:NVDA`, `xyz:TSLA`, etc. — not bare tickers. The SDK needs `perp_dexs=['', 'xyz']` passed to both `Info()` and `Exchange()` constructors to register xyz pairs in its internal `name_to_asset` / `coin_to_asset` mappings. Without this, `set_leverage("xyz:NVDA")` raises a KeyError. **Any bot trading across multiple HL dexes must initialize the SDK with all active dex identifiers.**

**Testnet xyz orderbooks are dead.** All 5 equity perps (NVDA, TSLA, AAPL, COIN, MSTR) have zero volume, null midPx, and empty L2 books on testnet. Oracle prices exist but no counterparties. Grid orders post but never fill. **Equity perp soak testing is only meaningful on mainnet.** Testnet is useful for validating multi-dex plumbing (leverage, mid routing, market hours gating) but not for P&L analysis.

**SDK 0.23.0 fixes the spot_meta testnet bug.** The `IndexError: list index out of range` on `Info.__init__` that required the `spot_meta={"universe": [], "tokens": []}` stub workaround is fixed in 0.23.0. The stub can be removed. **However, VCR cassettes recorded under 0.22.0 are incompatible** — the SDK now makes different initialization requests. Re-record all cassettes after upgrading.

**HL account abstraction modes affect balance reads.** HL now has three modes: default (old behavior), unifiedAccount (spot+perp combined), and portfolio. In unifiedAccount mode, `clearinghouseState.marginSummary.accountValue` returns 0 when there are no open perp positions — real balance lives in `spotClearinghouseState`. **Bots must check `userAbstraction` at startup and branch their equity-reading logic by mode.** ~~Original advice was to "fail fast if unified" — this was wrong.~~ As of 2026-05-28, HL defaults new accounts to unifiedAccount mode, so bots must support it. See the unified account lessons below.

**Soak v1 session analysis: crypto perps are most profitable overnight and weekends.** Net P&L by US equity market session: REGULAR (9:30–16:00 ET) = −$16.96/session, EXTENDED = +$2.59, OVERNIGHT = +$13.09, WEEKEND = +$15.26. Per-hour profitability was highest overnight ($0.30/hr) and weekends ($0.21/hr), worst during regular hours (−$0.48/hr). **Crypto perps should trade 24/7 with no market hours gating.** The existing `is_equity_perp()` guard correctly limits market hours to equity perps only — crypto was already ungated, the bot just hadn't been turned back on after the soak concluded.

## Unified Account + Mainnet Launch (osbot, 2026-05-28)

**Unified account equity: spot USDC total is the single source of truth.** In `unifiedAccount` mode, `spotClearinghouseState.balances[USDC].total` already includes margin held for open perp positions (reported separately as `hold`). The perp `clearinghouseState.accountValue` equals the held portion + unrealised PnL. **Adding spot total + perp accountValue double-counts the held margin.** Correct formula: just read spot USDC total. We shipped the double-count bug, saw $237 when HL UI showed $206, and fixed it within the same session.

**HL order responses can silently reject inside a 200 OK.** The HL `/exchange` endpoint returns HTTP 200 with `{"status": "ok", "response": {"data": {"statuses": [{"error": "..."}]}}}` for rejected orders. The SDK does not raise an exception — it returns the dict. **Bots must check `statuses[0]` for `"resting"` or `"filled"` to confirm an order landed.** Our bot was tracking 30 orders internally while only 10-13 were live on the exchange, because rejected orders were counted as submitted.

**$10 minimum notional rejects depend on szDecimals rounding.** The grid planner bumps per-level size to meet the $10 minimum, but the final size is rounded to `szDecimals` precision before submission. For SOL (szDecimals=2, price ~$165), 0.06 SOL = $9.90 — just below $10. The grid bumped it to 0.12 × $165 = $19.80 on paper, but the 80bps range at 16bps spacing puts some levels at prices where `round(size, 2) * level_price < $10`. **Min notional checks should use the rounded size × level price, not the base size × mid.**

**Running two bots (testnet + mainnet) from the same IP exhausts HL rate limits.** Both bots poll `allMids` and `user_state` every tick (~1s). Combined they exceeded the ~1200 req/min/IP budget, causing cascading 429s that stalled order placement and forager ranking. **Stop testnet bots before launching mainnet, or use separate IPs.**

**Graceful shutdown can leave orphaned child processes holding the health port.** `kill PID` sends SIGTERM to the main process, but the aiohttp health server runs in a child process (forked by nohup/shell wrapper) that doesn't receive the signal. The port stays bound, causing `OSError: address already in use` on restart. **Check `lsof -i :PORT` after shutdown and kill any lingering children before relaunching.**

## Grid Min-Notional Trap + Small-Capital Rework (osbot, 2026-06-03)

Bot halted itself 2026-06-02 on the 3% daily-loss breaker, leaving a stranded BTC long it could not unwind. Account flattened (BTC −$20.75, ETH −$0.82, xyz:NVDA −$0.59), −12.6% from $206 baseline. Root cause was structural, not a one-off bug.

**A grid MM has a minimum viable capital: `min_notional × grid_levels × n_active_pairs / WEL`.** Per-level size is `balance × (WEL / n_pairs) / levels`. At $200, WEL 0.15, 3 forager pairs, 5 levels → ~$9/level, **below the $10 floor**. Below this line the strategy can open but not properly close. For the live config that floor was **$1,000** while the account held $200 — a 5× capital-vs-constraints mismatch. Always compute this floor before deploying a grid; it scales with levels and pairs and inversely with WEL.

**Never "bump-and-trade" below min notional — refuse instead.** The old `grid.plan()` bumped sub-floor per-level notional up to $10 and kept trading. That oversizes the grid past the position cap (one-sided accumulation) AND makes the position un-unwindable: when the cap later forces all-reduce levels, each reduce order is itself below $10 and gets rejected, so the position can only grow until the daily-loss halt. Fix: a viability guard quotes **nothing (cancel-only)** when per-level notional `< 1.3× min_notional`. Headroom (1.3×) absorbs price drift + size rounding so a level placed at the floor doesn't fall below it before it can be reduced.

**Size rounding can drop a level below the floor even when the target cleared it.** `_round_size` rounds notional→coins to szDecimals with `round()`, which can round *down*; at a lower mark price the same coin size then falls under $10 and is rejected. Fix: after rounding, if `level_size × mid < min_notional`, bump up one tick. (Generalizes the 2026-05-28 SOL note to every pair.)

**The SDK returns a bare string on some error paths — `result.get(...)` then crashes the tick.** `_submit_one` assumed a dict response; a string error (e.g. min-notional) raised `AttributeError: 'str' object has no attribute 'get'`, killing the runner loop every tick. **Guard with `isinstance(result, dict)` before any `.get()`; treat non-dict as a rejection.**

**Default-dex info queries do NOT return HIP-3 dex positions.** A read-only `user_state()` on the default perp dex showed only BTC/ETH and missed an open `xyz:NVDA` equity-perp position on the `xyz` HIP-3 dex. When auditing/flattening, query each active dex explicitly or you'll under-count exposure. (`market_close` with `liquidation=None` in fills confirms a clean close vs. a forced liquidation.)

**Small-capital rework adopted.** Reworked the live config to 1 active pair (forager `top_n=1` over BTC/ETH only — dropped SOL and all `xyz:*` for gap risk), 3 levels, WEL 0.25 → $15/level at $180, viable across $180–500. Honest EV caveat: on BTC/ETH (tightest spreads, least edge) a small grid is realistically break-even-to-negative after fees; the genuine spread edge is on the equity perps that can't be run safely at this capital. This config is "viable + won't self-trap," not "reliably profitable."

## Secret-Free Config + Testnet Setup Gotchas (osbot, 2026-06-13 → 06-14)

**A config should live in version control with NO secrets; derive the address at runtime.** Made `account_address`/`keyfile_path`/`keyfile_password` optional and added `startup.resolve_account_address()`: explicit config value wins (agent mode), else derive from the signing wallet (master-key mode). Canonical configs now tracked at `configs/*.yaml` with auth from `HYPERLIQUID_{MAINNET,TESTNET}_PRIVATE_KEY` env. Kills the recurring config-drift problem where a hand-maintained `*.template` silently diverged from the live gitignored config for weeks.

**Manual discretionary trading on the bot's account corrupts its accounting.** A −$17 "mystery" drop on a flat account turned out to be hand-placed ZEC longs in the HL UI on the same account the bot runs on (fills had `cloid=None` + `crossed=True`; the bot stamps a cloid on every order). The bot's daily-loss baseline, equity tracking, and reconciliation all assume it's the only actor. **Give the bot a dedicated account/subaccount, or don't hand-trade the account while it runs.** To trace which actor traded an account, query by address (on-chain/API) — do NOT crawl sibling project folders.

**Testnet faucet is gated on mainnet activation → you fund the wrong wallet.** The HL testnet faucet (`/drip`) requires the claiming address to be mainnet-active. The dedicated testnet wallet (`0x3BaC`, $0 on mainnet) couldn't claim, so the user funded their mainnet-active address on testnet instead (same address, separate network). Result: the funded testnet account ≠ the bot's configured `HYPERLIQUID_TESTNET_PRIVATE_KEY` wallet. **Always verify the bot's resolved address actually matches the funded account before launch** (smoke-test prints it). The same private key signs on both networks, so the fix was to launch testnet signing with the mainnet key via an inline env override (`HYPERLIQUID_TESTNET_PRIVATE_KEY="$HYPERLIQUID_MAINNET_PRIVATE_KEY"`) — key never written to disk.

**Testnet accounts report `userAbstraction='disabled'`.** Startup only accepted `default`/`manual`/`unifiedAccount` and aborted on testnet. `disabled` = plain account, no abstraction → treat like `default` (non-unified, perp-collateral sizing). Added to the allowlist.

**Forager warmup spams `rank produced empty top_n` for `log_range_window_min` minutes.** Not an error — it needs a full range window before it can rank/select. Confirm candidate 24h volume clears `min_volume_usd_24h` (testnet volume can be thin) or it will warm up forever and never trade.

## Coin Selection — Grid Profitability Across the HL Universe (2026-06-14)

**A maker grid cannot be backtested from OHLC candles.** Built a faithful simulator (vol-scaled range, taker exits, live-calibrated fees) over 55 liquid perps; it predicted ETH **+11 bps/hr (positive)** while the *live testnet soak the same day was net negative*, and it did **2.4 fills/hr vs. live 24/hr** — a 10× miss. Root cause: maker fills depend on queue position and the intrabar price *path*, neither recoverable from 1m OHLC. The first cut even booked same-bar straddles (buy-the-low + sell-the-high in one candle = phantom profit, biased toward high-vol coins). **Takeaway: OHLC grid backtests are directional only at the extremes and worthless near zero; the only trustworthy test is a live testnet soak.** (Validated: the soak is what revealed ETH's true break-even.)

**The fee hurdle is ~5.5 bps/round-trip and it's the whole game.** Live fills confirm entries fill **maker @ 1.5 bps**, exits are **taker `market_close` @ 4.5 bps**. Against a 20 bps TP that's ~14.5 bps net per *winning* RT — but adverse-selected maker fills + trend exits make the average RT a wash. A coin only earns its place if it oscillates ≥~5.5 bps reliably without sustained trends.

**Liquidity and mean-reversion are anti-correlated in the HL perp universe.** The most liquid coins (BTC/ETH/SOL — high volume, high `active%`, VR(30)<1 mildly) trend more and mean-revert least; the strongly mean-reverting coins (KAITO VR 0.54, PUMP 0.71, kPEPE 0.75, TRX 0.52) are all $1–4M/day, where wide spreads break the 1.5/4.5 bps fee model. High volume ≠ tradeable: HYPE ($273M/day) ranks near the bottom because VR≈1.01 and it's trend-paused 73% of the time. **The forager's `log_range × volume` score captures vol×liquidity but ignores the trend-vs-revert axis, which is the actual edge discriminator.** Best untested liquid candidate: **SOL** — add it to `forager.candidates` and testnet-soak before any conclusion. Full analysis: [docs/coin-selection-analysis.md](coin-selection-analysis.md).

**Adding a coin to `forager.candidates` does NOT mean it gets traded — `top_n=1` + a volume-weighted score starves lower-volume coins.** Added SOL to the testnet candidates and restarted (2026-06-14 19:22); over the next ~4.7h the forager traded ETH (52 fills) + a little BTC but **SOL never won the slot (0 fills).** The score is `log_range × volume`, so ETH ($688M/day) and BTC ($1.3B) dominate the single `top_n=1` slot unless SOL's ($117M) volatility spikes ~6× to compensate on the `log_range` term — which it doesn't. **To actually test a lower-volume candidate, either raise `top_n` (trade the top 2–3) or temporarily restrict `candidate_pairs` to force it.** Mechanically the restart was clean (graceful stop cancelled 6 orders, 3 pairs prepared) — the gap is the selection math, not the plumbing.

## Stop-Loss A/B + SOL/BTC Verdict + v4 Fee Discipline (osbot, 2026-06-15 → 06-26)

**VERDICT: this passive grid is a NET LOSER on liquid majors — gross edge sits below fee + stop-slippage, so calm-regime gains never survive trend-regime losses.** SOL dedicated soak: net −6 bps over 210 RTs. BTC pivot (the proven-better coin) with every fix applied: net trajectory **+$0.54 (50 RTs) → −$0.03 (74) → −$0.70 (90)**, account $196.5→$195.2. Both coins, full multi-regime samples, net-negative. The mechanics (stop, post-only, SL-only, funding-aware) all work — they just can't manufacture edge that isn't there. **Majors are not the path; only wider-spread mainnet xyz equity perps can clear the fee.** Don't re-test majors.

**Small/calm samples systematically mislead — it looked profitable FOUR separate times and reverted every time** (SOL +1.8→−6 bps; BTC +15bps@5RT → +$0.60@50RT → breakeven@74 → −$0.70@90). Each *larger* sample was worse. A grid in a calm/ranging window captures clean spread and looks great; the truth only emerges once sustained trends are sampled. **Never call a verdict before ~150–200 RTs spanning both calm and trending regimes.** I over-called "net-positive" at 50 RTs — corrected twice as the sample grew.

**An inventory stop on a grid MM must sit OUTSIDE the grid's operating range, or it whipsaws.** A 25 bps stop *inside* the 40 bps grid range fired on routine intra-grid mean-reversion swings → **42% stop-rate in a CALM market**, net −7.2 bps (WORSE than no effective stop). Moving it to 55–65 bps (outside the range) eliminated the whipsaw (SL rate ~9% → only fires on real trends). **A backtest/replay that lacks the order book CANNOT detect whipsaw** — the replay of the 25 bps stop predicted +$1.4 (it only sees realized fills, not the would-have-reverted positions it cuts); only the live soak revealed the whipsaw. Only a live soak detects "cut-winner" stop pathologies.

**Stops slip badly in fast moves (gap-through-stop).** The 65 bps stop FIRED at threshold but the `market_close` filled ~143 bps adverse in a 1.4% BTC move — the stop caps the *tail* but not the *gap*. Real tail risk remains even with a working stop; a scaled/partial exit or vol-scaled spacing would help.

**Maker rebates are UNREACHABLE at micro scale — verified, and it corrects a load-bearing prior assumption.** HL's −0.003% maker rebate needs **14-day volume > $500M**; a $300–600 bot does ~$30–100k/14d. The 129h-soak doc's "+138% from maker rebates" projection assumed a −0.002% maker that micro-sizing can't reach. **Real fee floor = ~1.5 bps maker (3 bps/RT), no rebate. The profitability bar is gross edge > ~3 bps/RT.** (HYPE-staking discounts ≤5–15% at reachable levels = negligible.)

**The PnL measurement was FUNDING-BLIND (net = closedPnl − fees).** Fine for majors (funding ~0) but materially wrong for the xyz profit gate where funding is +13–20% APY — HL pays funding *separately* from `closedPnl`, so a long-leaning grid bleeds funding the trade-PnL view never shows. Built `tools/funding_pnl.py` (folds HL `userFunding` into net + a time-weighted inventory-lean readout: long-pays / short-earns) and wired it into `coin_watch.py`. Confirmed grid runs slightly short on majors (earns funding). Instrument BEFORE risking real money on a high-funding market.

**v4 fee discipline works as designed:** added `StrategyConfig.post_only` → grid quotes placed as `Alo` (post-only) via `grid.py order_tif`. Live: held grid-ENTRY taker% at **0.0% and fees at the 3 bps/RT all-maker floor, 0 ALO rejects** (a few rejects only appeared in fast moves). SL/TTL flattens are taker by design (so taker% ticks up per such exit — NOT post-only breaking). `tp_pct=0.1` makes the `TripleBarrier` effectively **SL-only** (the grid's own maker sell levels take profits instead of the ExitManager closing as taker), removing a ~0.7 bps/RT taker drag.

**Ops/tooling gotchas this session:**
- **Shadow-DB write-lag is large (seen up to ~7h)** — a stale `last fill` timestamp is NOT a stall. Trust the live health `account_value` for real-time state; fills are tagged by *exchange* time so late flushes still attribute to the right before/after segment.
- **stdout is block-buffered under `nohup`** — the logfile freezes while the process runs fine. Trust health/shadow-DB, not the apparent log tail. Launch with `python -u` / `PYTHONUNBUFFERED=1` when you need live logs.
- **`read -t N </dev/null` is a NO-OP, not a sleep** — reading EOF from /dev/null returns instantly, so "wait" loops built on it don't wait (caused repeated too-early checks). For a real bounded wait without foreground `sleep`, use `timeout N tail -f /dev/null`, or `timeout N grep -m1 PATTERN <(tail -f LOG)` to block until a log line appears.
- **`pkill -f <pattern>` self-matches the calling shell** if the pattern string is in the shell's own command line (e.g. the config filename) → it kills your shell (exit 144). Kill by PID instead.
- **The funded testnet account `0x0d3B` is controlled by the MAINNET key**, not the `.env` testnet key (which derives a different, empty `0x3BaC` wallet → "accountValue not positive: 0.0"). Launch testnet bots with an inline override: `export HYPERLIQUID_TESTNET_PRIVATE_KEY="$HYPERLIQUID_MAINNET_PRIVATE_KEY"` (see memory reference_testnet_wallet).
